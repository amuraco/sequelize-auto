...
AutoSequelize.prototype.build = function(callback) {
  ...
  function processTables(__tables) {
    if (self.sequelize.options.dialect === 'mssql')
      __tables = _.map(__tables, 'tableName').sort();
...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      text[table] += "\n" + spaces + "});";
      if(self.options.typescript) typescriptFiles[0] += tsHelper.def.getDefinitionModel(tableName);
      if(self.options.typescript) text[table] = tsHelper.model.getModelFileStart(self.options.indentation, spaces, tableName) + '\n'+ tsHelper.def.getTableDefinition(text[table], tsTableDef, tableName);
      ...
      if(self.options.typescript) typescriptFiles[0] += `
export default { 
\t${_.keys(text).join(',\n\t')}
};`;
...
AutoSequelize.prototype.write = function(attributes, typescriptFiles, callback) {
  ...
  if (self.options.typescript) {
    if (typescriptFiles !== null && typescriptFiles.length > 1) {
      fs.writeFileSync(path.join(self.options.directory, 'db.types.ts'), typescriptFiles[0], 'utf8');
      fs.writeFileSync(path.join(self.options.directory, 'db.models.ts'), typescriptFiles[1], 'utf8');
    }
  }
